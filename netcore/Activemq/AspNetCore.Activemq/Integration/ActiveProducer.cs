...
namespace AspNetCore.Activemq.Integration
{
    public class ActiveProducer : ActiveBase
    {
        ...
        public ActiveProducer(bool isCluster, string[] hostAndPorts, string query = null, ActiveProtocol protocol = ActiveProtocol.OpenWrite) :
            base(isCluster, hostAndPorts, query, protocol)
        {
        ...
        private void ReturnConnection(IConnection connection)
        {
            ...
            {
                if (connectionQueue.Count < InitializeCount && connection != null && IsConnected(connection))
                {
        ...
        {
            return new ActiveProducer(activeBaseOptions.IsCluster, activeBaseOptions.BrokerUris, activeBaseOptions.Query, activeBaseOptions.Protocol)
            {
