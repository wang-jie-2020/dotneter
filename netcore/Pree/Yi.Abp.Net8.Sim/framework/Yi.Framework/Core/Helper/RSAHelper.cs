...
public class RSAHelper
{
    ...
    public string Decrypt(string cipherText)
    {
        ...
        var buffer = new byte[bufferSize]; //待解密块  
        using (var msInput = new MemoryStream(Convert.FromBase64String(cipherText)))
        {
            using (var msOutput = new MemoryStream())
            {
                ...
                while ((readLen = msInput.Read(buffer, 0, bufferSize)) > 0)
                {
                    ...
                    var encData = _privateKeyRsaProvider.Decrypt(dataToEnc, RSAEncryptionPadding.Pkcs1);
    ...
    public string Encrypt(string text)
    {
        ...
        var buffer = new byte[bufferSize]; //待加密块

        using (var msInput = new MemoryStream(_encoding.GetBytes(text)))
        {
            using (var msOutput = new MemoryStream())
            {
                int readLen;
                while ((readLen = msInput.Read(buffer, 0, bufferSize)) > 0)
                {
                    var dataToEnc = new byte[readLen];
                    Array.Copy(buffer, 0, dataToEnc, 0, readLen);
                    var encData = _publicKeyRsaProvider.Encrypt(dataToEnc, RSAEncryptionPadding.Pkcs1);
                    msOutput.Write(encData, 0, encData.Length);
                }
