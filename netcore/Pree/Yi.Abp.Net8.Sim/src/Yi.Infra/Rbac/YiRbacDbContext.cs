    ...
    public class YiRbacDbContext : SqlSugarDbContext
    {
        ...
        protected void DataPermissionFilter(ISqlSugarClient sqlSugarClient)
        {
            ...
            if (/*CurrentUser.GetDeptId() is null ||*/ roleInfo is null)
            {
                ...
                expRole.Or(it => Enumerable.Select<RoleTokenInfoModel, Guid>(roleInfo, x => x.Id).Contains(it.Id));
            }
            else
            {
                foreach (var role in Enumerable.OrderBy<RoleTokenInfoModel, DataScopeEnum>(roleInfo, f => f.DataScope))
                {
