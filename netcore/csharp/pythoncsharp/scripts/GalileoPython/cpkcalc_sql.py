...
def fetch_sql_list(dbset, sql):
    dbsetting = json.loads(dbset.replace("'", '"'))
...
def calc_onebar_chart_old(sigma_overall,average,usl,lsl,value_data):
    # print("#######usl")
    # print(usl)
    # print("#######usl")
    # print("#######lsl")
    ...
    while(x_start <= x_end):
        ...
        # 取6位有效数字
        x_array.append(float(format_float(x_start)))
        y_array.append(0)
        i_max+=1
    ...
    for v in value_data:
        for x_index in range(x_startindex,x_len):
            ...
            x_min = x_array[x_index]
            x_max = x_array[x_index+1] if (x_index+1) <= (x_len-1) else x_array[x_index] + x_interval / 2
...
def calc_onebar_chart(sigma_overall,average,usl,lsl,value_data):
    ...
    # 计算分许颗粒度
    num_bins = freedman_diaconis_bins(np.sort(value_data))
    # 生成横轴数据平均分布
    x_array = np.linspace(average - 6 * sigma_overall, average + 6 * sigma_overall, num_bins).tolist()

    y_array = [0] * len(x_array)  # 初始化一个和list_1一样长度的数组，用于存放统计结果
    for i in range(len(x_array) - 1):  # 遍历list_1的每个区间
        lower_bound = x_array[i]
        upper_bound = x_array[i + 1]
        count = sum(1 for x in value_data if lower_bound <= x < upper_bound)  # 统计list_2中落在当前区间内的数据个数
        y_array[i] = count
...
def freedman_diaconis_bins(data):
    ...
    num_bins = int(np.round(data_range / bin_width))+1
    if num_bins < 10:
        num_bins = 10
    elif num_bins > 1000:
        num_bins = 1000
...
# 伽利略系统计算的主函数
# Parameters:
#   base_dbset: mysql库 连接字符串
#   sql_dbset: 执行SQL的库 连接字符串
...
# group_data_all数据
def calc(base_dbset,sql_dbset,sqlId,usl_value,lsl_value,table_value_key,table_group_key,group_mincount):
    usl=""
    lsl=""
    if len(usl_value) > 0:
        usl = float(usl_value)
    ...
    base_dbset = base_dbset.replace("'", "\"")
    sqldatatable_sql = "select * from GALILEO_MODELRECIPEDETAILANALYSISRESULT where id = '" + sqlId + "'" 
    sqldatatable = fetch_sql_list(base_dbset,sqldatatable_sql)
    sqls = ""
    ...
    # print(sql)
    data_list = fetch_sql_list(sql_dbset,sql) #获取到分析结果的所有数据
    # print(data_list)
    ...
    for data_list_item in data_list:
        ...
        # 如果是decimial类型，转为float
        if(isinstance(data_list_item[table_value_key], Decimal)):
            data_list_item[table_value_key] = float(data_list_item[table_value_key])
    ...
    global group_point_dataa_array
    group_point_dataa_array = []
    ...
    for groupdetail in group_list:
        ...
        if do_calc == 1:
            ...
            for groupvalue in groupdetail:
                ...
                # 散点图数据，添加组内的所有点
                group_point_dataa_array.append([grindex,groupvalue])
                # print("groupvalue")
    ...
    group_dataa_array_all["point_data"] = group_point_dataa_array
...
if __name__=='__main__':
    try:
        #代码行
        base_dbset = sys.argv[1] # 2024.02.01 修改,增加当前mysql连接字符串
        sql_dbset = sys.argv[2]
        sql = sys.argv[3]
        usl = sys.argv[4]
        lsl = sys.argv[5]
        table_value_key = sys.argv[6]
        table_group_key = sys.argv[7]
        mincount = float(sys.argv[8])
        
        # snrate_calc = float(sys.argv[8])
        # swtest_calc = float(sys.argv[9])
        # kstest_calc = float(sys.argv[10])
        # result = calc(dbset,sql,usl,lsl,table_value_key,table_group_key,mincount,snrate_calc,swtest_calc,kstest_calc)
        # OUTPUTLOTNO_key = sys.argv[8]
        # TOTALPROCESSED_key = sys.argv[9]
        result = calc(base_dbset,sql_dbset,sql,usl,lsl,table_value_key,table_group_key,mincount)
        # 测试
        #  "23c3141c-9f35-4552-975d-ff7f3a4c5d1e" "200" "0" "VALUE" "GroupColumn" "20" "1" "1" "1"   -u
        # result = calc("{'server':'10.206.129.101','port':3306,'userid':'GalileoDB_prd','password':'HBSY256eda#$234','database':'GalileoDB_prd'}", "f5858108-69f5-496c-bbb5-e2f148689622", "1125", "1115", "VALUE", "GroupColumn", "1" )
        # doris测试
        # result = calc("{'server':'10.206.134.63','port':3306,'userid':'root','password':'Qwer12343!@','database':'GalileoDB_prd'}", "{'server':'10.205.128.107','port':9030,'userid':'eda_user','password':'Eda_user_1234','database':'ordos_eda_dws'}", "5a356064-c054-49e8-870b-e6439f972660", "", "20", "VALUE", "GroupColumn", "1" )
