...
namespace Demo
{
    public class DatabaseInitializer2
    {
        public static void Seed(IApplicationBuilder app)
        {
            ...
            {
                wipOrderCommands1.Add($"UPDATE [dbo].[WipOrder_RemoveIndex] SET [WorkOrderStatus] = N'待测' WHERE WipOrderId = {i}");
            }

            var wipOperationCommands1 = new List<string>();
            for (var i = index1; i < index2; i++)
            {
                wipOperationCommands1.Add($"UPDATE [dbo].[WipOperation_RemoveIndex] SET [OperationStatus] = N'已排程' WHERE WipOrderId = {i}");
            }

            var wipOrderCommands2 = new List<string>();
            for (var i = index1; i < index2; i++)
            {
                wipOrderCommands2.Add($"UPDATE [dbo].[WipOrder_RemoveIndex] SET [WorkOrderStatus] = N'在测' WHERE WipOrderId = {i}");
            }

            var wipOperationCommands2 = new List<string>();
            for (var i = index1; i < index2; i++)
            {
                wipOperationCommands2.Add($"UPDATE [dbo].[WipOperation_RemoveIndex] SET [OperationStatus] = N'未排程' WHERE WipOrderId = {i}");
            }
        ...
        private const string querySQL = @"
            ...
            FROM
             ...
             FROM
              ...
              WHERE
               ...
               SELECT
                1 
               FROM
                [WipOperation_RemoveIndex] AS [w1] 
               WHERE
             ...
             SELECT
              ...
              [w2].[WipOrderID] 
             FROM
              [WipOperation_RemoveIndex] AS [w2] 
             WHERE
        ...
        private const string updateSQL1 = @"
            UPDATE [dbo].[WipOrder_RemoveIndex] SET [WorkOrderStatus] = N'待测' WHERE WipOrderId > 241105 AND WipOrderId < 242834;
            WAITFOR DELAY '00:00:05';
            UPDATE [dbo].[WipOperation_RemoveIndex] SET [OperationStatus] = N'已排程' WHERE WipOrderId > 241105 AND WipOrderId < 242834;";

        private const string updateSQL2 = @"
            UPDATE [dbo].[WipOrder_RemoveIndex] SET [WorkOrderStatus] = N'在测' WHERE WipOrderId > 241105 AND WipOrderId < 242834;
            WAITFOR DELAY '00:00:05';
            UPDATE [dbo].[WipOperation_RemoveIndex] SET [OperationStatus] = N'未排程' WHERE WipOrderId > 241105 AND WipOrderId < 242834;";

        private const string insertSQL1 = @"INSERT INTO [dbo].[WipOrder_RemoveIndex] (
	                [WipOrderGroupId],
	                [ProductID],
	                [CompletedQuantity],
	                [WorkOrderStatus],
        ...
        private const string insertSQL2 = @"INSERT INTO [dbo].[WipOperation_RemoveIndex] (
	            [WipOrderID],
	            [ProcessId],
	            [ProcessOperationId],
	            [OperationId],
