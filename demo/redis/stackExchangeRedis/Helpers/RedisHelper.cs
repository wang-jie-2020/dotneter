...
namespace Demo.Helpers
{
    ...
    public class RedisHelper
    {
        ...
        public RedisHelper(ConnectionMultiplexer conn) : this(conn, 0, "")
        {

        }

        public RedisHelper(ConnectionMultiplexer conn, int dbNum) : this(conn, dbNum, "")
        {

        }

        public RedisHelper(ConnectionMultiplexer conn, int dbNum, string prefixKey)
        {
            _conn = conn;
            _dbNum = dbNum;
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public List<T> ListTrim<T>(string key, long start = 0, long stop = -1)
        {
            key = AddSysCustomKey(key);
            return Do(redis =>
            {
                redis.ListTrim(key, start, stop);
                var values = redis.ListRange(key);
                return ConvetList<T>(values);
            });
        }
