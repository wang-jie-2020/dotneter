...
namespace basic.RSA
{
    public sealed class CertificateHelper
    {
        ...
        /// <summary> 
        /// 根据指定的证书名和certPath全路径生成证书（包含公钥和私钥，并保存在MY存储区） 
        /// </summary> 
        /// <param name="subjectName"></param> 
        /// <param name="certPath"></param> 
        /// <returns></returns> 
        public static bool CreateCertWithPrivateKey(string subjectName, string certPath)
        {
            subjectName = "CN=" + subjectName;
            string param = " -pe -ss my -n /" + subjectName + "/";
            try
            {
                Process p = Process.Start(certPath, param);
                p.WaitForExit();
                p.Close();
            }
            catch (Exception e)
