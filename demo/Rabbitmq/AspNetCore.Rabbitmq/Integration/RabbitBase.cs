...
namespace AspNetCore.Rabbitmq.Integration
{
    public abstract class RabbitBase : IDisposable
    {
        ...
        static Type basicPropertiesType;
        static RabbitBase()
        {
            basicPropertiesType = typeof(RabbitMQ.Client.AmqpTcpEndpoint).Assembly.GetTypes().FirstOrDefault(f => f.FullName == "RabbitMQ.Client.Framing.BasicProperties") ?? throw new ArgumentNullException(nameof(basicPropertiesType));
        }
        ...
        protected IBasicProperties CreateBasicProperties(MessageOptions messageOptions = null)
        {
            if (messageOptions == null)
            {
                return null;
            }
            var prop = (IBasicProperties)Activator.CreateInstance(basicPropertiesType);
            prop.AppId = messageOptions.AppId;
            prop.ContentEncoding = messageOptions.ContentEncoding;
            prop.ContentType = messageOptions.ContentType;
            prop.CorrelationId = messageOptions.CorrelationId;
            prop.Expiration = messageOptions.Expiration;
            prop.Headers = messageOptions.Headers;
            prop.MessageId = messageOptions.MessageId;
            prop.Persistent = messageOptions.Persistent;
            prop.Priority = messageOptions.Priority;
            prop.Type = messageOptions.Type;
            prop.UserId = messageOptions.UserId;
            return prop;
        }
    }
}
